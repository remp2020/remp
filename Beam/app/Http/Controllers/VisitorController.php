...
class VisitorController extends Controller
{
    ...
    public function dtBrowsers(Request $request, Datatables $datatables)
    {
        ...
        if ($request->input('visited_from')) {
            $devices->where('time_from', '>=', Carbon::parse($request->input('visited_from'), $request->input('tz'))->tz('UTC'));
        }
        if ($request->input('visited_to')) {
            $devices->where('time_to', '<=', Carbon::parse($request->input('visited_to'), $request->input('tz'))->tz('UTC'));
        }
