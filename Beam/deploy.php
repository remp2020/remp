...
task('deploy:tmplink', function() {
    ...
    run("ln -s /tmp/remp_beam_cache {{release_path}}/storage/framework/cache");

    run("rm -fr {{release_path}}/storage/framework/sessions");
    run("ln -s /tmp/remp_beam_sessions {{release_path}}/storage/framework/sessions");

    run("rm -fr {{release_path}}/storage/framework/testing");
    run("ln -s /tmp/remp_beam_testing {{release_path}}/storage/framework/testing");

    run("rm -fr {{release_path}}/storage/framework/views");
    run("ln -s /tmp/remp_beam_views {{release_path}}/storage/framework/views");
})->desc('Temp symlink');
