...
class VisitorController extends Controller
{
    ...
    {
        return response()->view('beam::visitors.devices', [
    ...
    {
        return response()->view('beam::visitors.sources', [
            'visitedFrom' => $request->input('visited_from', 'today - 30 days'),
            'visitedTo' => $request->input('visited_to', 'now'),
            'subscriber' => $request->input('subscriber', "1"),
    ...
    public function dtBrowsers(Request $request, Datatables $datatables)
    {
        ...
        if ($request->input('visited_from')) {
            $devices->where('time_from', '>=', Carbon::parse($request->input('visited_from'), $request->input('tz')));
        }
        if ($request->input('visited_to')) {
            $devices->where('time_to', '<=', Carbon::parse($request->input('visited_to'), $request->input('tz')));
        }
